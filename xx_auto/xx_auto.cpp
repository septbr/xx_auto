...
namespace xx_auto // window
{
    ...
    __point<T> __point<T>::operator-(const __point &other) const { return __point{x - other.x, y - other.y}; }
    template <typename T>
    __point<T> __point<T>::operator*(T value) const { return __point{x * value, y * value}; }
    template <typename T>
    __point<T> __point<T>::operator/(T value) const { return __point{x / value, y / value}; }
    template <typename T>
    __point<T> &__point<T>::operator+=(const __point &other) {
    ...
    template <typename T>
    __point<T> &__point<T>::operator*=(T value) {
        x *= value;
        y *= value;
        return *this;
    }
    ...
    bool menu::append(unsigned int id, const std::wstring &title) const { return AppendMenuW(_hmenu, MF_STRING, id, title.c_str()); }
    ...
    bool menu::insert(unsigned int where, unsigned int id, const std::wstring &title, bool use_position) const { return InsertMenuW(_hmenu, where, MF_STRING | (use_position ? MF_BYPOSITION : MF_BYCOMMAND), id, title.c_str()); }
    bool menu::insert(unsigned int where, const menu &submenu, const std::wstring &title, bool use_position) const { return InsertMenuW(_hmenu, where, MF_STRING | MF_POPUP | (use_position ? MF_BYPOSITION : MF_BYCOMMAND), (UINT_PTR)submenu.hmenu(), title.c_str()); }
    bool menu::insert(unsigned int where, bool use_position) const { return InsertMenuW(_hmenu, where, MF_SEPARATOR | (use_position ? MF_BYPOSITION : MF_BYCOMMAND), 0, nullptr); }
    bool menu::remove(unsigned int where, bool use_position) const { return RemoveMenu(_hmenu, where, use_position ? MF_BYPOSITION : MF_BYCOMMAND); }
    bool menu::enable(unsigned int where, bool enabled, bool use_position) const { return EnableMenuItem(_hmenu, where, (enabled ? MF_ENABLED : MF_DISABLED | MF_GRAYED) | (use_position ? MF_BYPOSITION : MF_BYCOMMAND)); }
    bool menu::modify(unsigned int where, const std::wstring &title, bool use_position) const { return ModifyMenuW(_hmenu, where, MF_STRING | (use_position ? MF_BYPOSITION : MF_BYCOMMAND), where, title.c_str()); }
    bool menu::check(unsigned int where, bool checked, bool use_position) const { return CheckMenuItem(_hmenu, where, (checked ? MF_CHECKED : MF_UNCHECKED) | (use_position ? MF_BYPOSITION : MF_BYCOMMAND)); }
    ...
    long window::style(long style, value_editor setter, bool ext) const {
        switch (setter) {
        case value_editor::replace: return SetWindowLongW(_hwnd, ext ? GWL_EXSTYLE : GWL_STYLE, style);
        case value_editor::append: return SetWindowLongW(_hwnd, ext ? GWL_EXSTYLE : GWL_STYLE, style | this->style(ext));
        case value_editor::remove: return SetWindowLongW(_hwnd, ext ? GWL_EXSTYLE : GWL_STYLE, style & ~this->style(ext));
        }
        return 0;
    }
    xx_auto::rect window::rect(bool client) const {
        ...
        return xx_auto::rect{static_cast<int>(rct.left), static_cast<int>(rct.top), static_cast<int>(rct.right - rct.left), static_cast<int>(rct.bottom - rct.top)};
    }
    xx_auto::rect window::rect() const { return rect(false); }
    bool window::rect(int x, int y, int width, int height) const { return SetWindowPos(_hwnd, nullptr, x, y, width, height, SWP_NOZORDER | SWP_NOACTIVATE); }
    bool window::rect(const xx_auto::rect &rect) const { return this->rect(rect.x, rect.y, rect.width, rect.height); }
    point window::position() const {
        auto rect = this->rect();
        return point{rect.x, rect.y};
    }
    bool window::position(int x, int y) const { return SetWindowPos(_hwnd, nullptr, x, y, 0, 0, SWP_NOSIZE | SWP_NOZORDER | SWP_NOACTIVATE); }
    bool window::position(const xx_auto::point &point) const { return position(point.x, point.y); }
    bool window::resize(int width, int height) const { return SetWindowPos(_hwnd, nullptr, 0, 0, width, height, SWP_NOMOVE | SWP_NOZORDER | SWP_NOACTIVATE); }
    point window::screen_point(int x, int y) const {
        POINT pt = {x, y};
        ClientToScreen(_hwnd, &pt);
        return point{static_cast<int>(pt.x), static_cast<int>(pt.y)};
    }
    point window::screen_point(const point &client_point) const { return screen_point(client_point.x, client_point.y); }
    point window::client_point(int x, int y) const {
        POINT pt = {x, y};
        ScreenToClient(_hwnd, &pt);
        return point{static_cast<int>(pt.x), static_cast<int>(pt.y)};
    }
    point window::client_point(const point &screen_point) const { return client_point(screen_point.x, screen_point.y); }
    bool window::click(int x, int y) const {
        auto from = MAKELPARAM(x, y);
        return PostMessageW(_hwnd, WM_LBUTTONDOWN, MK_LBUTTON, from) && PostMessageW(_hwnd, WM_LBUTTONUP, MK_LBUTTON, from);
    }
    bool window::click(const xx_auto::point &point) const { return click(point.x, point.y); }

    bool window::drag(int x, int y, int horizontal, int vertical) const {
        auto from = MAKELPARAM(x, y), to = MAKELPARAM(x + horizontal, y + vertical);
        return PostMessageW(_hwnd, WM_LBUTTONDOWN, MK_LBUTTON, from) && PostMessageW(_hwnd, WM_MOUSEMOVE, MK_LBUTTON, to) && PostMessageW(_hwnd, WM_LBUTTONUP, MK_LBUTTON, to);
    }
    bool window::drag(const xx_auto::point &point, int horizontal, int vertical) const { return drag(point.x, point.y, horizontal, vertical); }
    bool window::drag(int x, int y, const xx_auto::point &direction) const { return drag(x, y, direction.x, direction.y); }
    bool window::drag(const xx_auto::point &from, const xx_auto::point &direction) const { return drag(from.x, from.y, direction.x, direction.y); }
